...
ログとクラッシュ dump は次のフォルダに保存されます。

```text
%LocalAppData%\cMemo\logs
```

ログは `cMemo.log` に出力され、1MB を超えた場合は最大 5 世代までローテーションします。
メモ本文はログに出力しません。ヘルプメニューの `ログフォルダを開く` から保存場所を開けます。
...
gcc src/main.c src/settings.c src/logger.c dist/app.res -municode -mwindows -lcomdlg32 -ldwmapi -lshell32 -lgdi32 -lwininet -ldbghelp -o dist/cMemo.exe
```
